...
## v1.1.2 - 2025年11月1日 - 初期化コマンドの追加とCSV読み込みの改善

### 変更内容

#### 新機能: `pairplot init` コマンド
- **ワークスペース初期化**: `data/`と`output/`フォルダを自動作成
- **サンプルデータの自動生成**: 
  - `sample_data1.csv` (タブ区切り)
  - `sample_data2.csv` (カンマ区切り)
- **ユーザビリティの向上**: 手動でフォルダを作成する手間を削減
- **既存フォルダの検出**: 既にフォルダが存在する場合は警告なしでスキップ
- **すぐに試せる**: サンプルデータで即座にライブラリの機能をテスト可能

#### CSV読み込みの堅牢性向上
- **末尾の余分な区切り文字に対応**: カンマやタブが行末に余分にあっても正常に読み込み
- **空白列の自動削除**: `Unnamed`列を自動的に除去
- **より明確なフィードバック**: 読み込み時に使用された区切り文字を表示
- **複数の区切り文字に対応**: 
  - カンマ区切り (`,`)
  - タブ区切り (`\t`)
  - セミコロン区切り (`;`)
...
```
============================================================
Pairplot Library - ワークスペース初期化
============================================================

カレントディレクトリに data/ と output/ フォルダを作成します。
カレントディレクトリ: /path/to/my_analysis

✓ data/ フォルダを作成しました
✓ output/ フォルダを作成しました

サンプルデータファイルを作成中...
  ✓ sample_data1.csv を作成しました（タブ区切り）
  ✓ sample_data2.csv を作成しました（カンマ区切り）
...
1. サンプルデータで試す:
   pairplot
   → sample_data1.csv または sample_data2.csv を選択
...
#### ドキュメントの更新
- **README.md**: `pairplot init`コマンドの説明を追加
- **QUICKSTART.md**: クイックスタートガイドに初期化コマンドを統合

### ファイル変更
- `cli.py`: `init_workspace()`関数を拡張、サンプルデータ生成を統合
- `core/sample_data.py`: サンプルデータ生成モジュールを新規作成
- `core/data_loader.py`: `load_csv_robust()`関数を大幅に改善
- `__init__.py`: バージョンを1.1.2に更新
- `setup.py`: バージョンを1.1.2に更新
- `README.md`: 使い方セクションを更新
- `QUICKSTART.md`: ステップ1を更新

### 技術詳細
- コマンドライン引数`init`を検出して初期化処理を実行
- `os.makedirs()`を使用してフォルダを作成
- 既存フォルダがある場合は作成をスキップ（エラーなし）
- サンプルデータは文字列定数から生成
- `skipinitialspace=True`で空白を自動トリミング
- `skip_blank_lines=True`で空行をスキップ
- 正規表現で`Unnamed`列を検出・削除

### 下位互換性
- 既存の`pairplot`コマンド（引数なし）は完全に互換性を保持
- 既存のワークフローに影響なし
- すべてのCSV読み込み機能は後方互換性を維持
